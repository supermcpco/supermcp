...
// Which of the configured master keys an operation used, as reported to
// a KEKObserver.
const (
	KeyActive   = "active"
...
// provider (ProviderLocal, ProviderAWSKMS), KeyActive or KeyPrevious,
// OpWrap or OpUnwrap, and the error, nil on success. The key role is what
// tells a move's old KMS key failing apart from the new one: during a
// move both are awskms.
type KEKObserver func(provider, key, op string, err error)
...
//
// key is KeyActive or KeyPrevious, the role k plays in the process.
func Observed(k KEK, key string, observe KEKObserver) KEK {
	if k == nil || observe == nil {
		return k
	}
	return &observedKEK{KEK: k, provider: ProviderOf(k.Ref()), key: key, observe: observe}
}
...
func (o *observedKEK) forRef(ref string) (KEK, bool) {
	...
	return &observedKEK{KEK: k, provider: o.provider, key: o.key, observe: o.observe}, true
}
...
func (o *observedKEK) report(ctx context.Context, op string, err error) {
	...
	o.observe(o.provider, o.key, op, err)
}
