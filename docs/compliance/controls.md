...
| Source code is analysed for flaws | CodeQL runs the `security-and-quality` queries over the Go code and the web UI's TypeScript on every pull request to `main`, every push to `main` and weekly, and reports to code scanning under its own category per language. Its findings are reported, not a merge gate. | Yes |
...
| Schema changes are safe to roll | Migrations run under an advisory lock before the new pods start, `/readyz` fails while the schema is behind the binary, and `scripts/check-migrations.sh`, run in the `go-lint` CI job, refuses a migration whose Up section drops a table, column or index, renames, changes a column's type, sets or adds a column `NOT NULL` without a default, truncates or deletes rows, unless the migration carries a `-- supermcp:breaking` line and `docs/UPGRADING.md` names its number. Down sections are not checked. | Yes |
