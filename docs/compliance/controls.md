...
| Joiner, mover, leaver | SCIM 2.0 creates, updates and deactivates accounts. `active: false` revokes that person's sessions, API keys and refresh tokens immediately. Single sign-on reconciles provider-sourced role bindings to the person's groups on every sign-in, leaving manual bindings alone. | Yes |
| Access review | `GET /api/v1/roles` reports who holds each role and `GET /api/v1/roles/{id}/bindings` lists them. **Partial:** there is no attestation workflow, no report of dormant accounts or unused credentials, and no `compliance access-review` command. |
...
| Records reach a system the subject cannot edit | A webhook exporter ships the stream as newline-delimited JSON, at-least-once with a cursor, each delivery signed with HMAC-SHA256 over a timestamp and the body. **Partial:** there is no API or screen to configure an exporter — rows must be inserted into `audit_exporters` directly — and webhook is the only kind. No syslog, no object storage, no OTLP, no Splunk. |
...
| Logging failures are visible | The queue depth is published as a metric with an alert rule. Events the queue could not take are counted and reported into the stream itself on the next successful append, because a gap nobody records is indistinguishable from a quiet period. | Yes |
...
| Keys held outside the application | AWS KMS, where the data key reaches AWS only as one `Encrypt` or `Decrypt` call and no master key exists in the pod. **Partial:** AWS KMS is the only external provider. GCP KMS, Azure Key Vault and Vault transit are accepted by the Helm chart and **not implemented by the binary**; configuring one produces a crash loop. The chart also renders the wrong variable names for AWS KMS — see `docs/install.md`. |
| Key separation between environments | The KMS encryption context names the installation, so a wrapped key lifted from one database cannot be unwrapped against another that shares the AWS key. Set `SUPERMCP_KMS_DEPLOYMENT`. | Yes |
...
| Signing key rotation | **Not available.** The keyring supports publishing a next key before use and promoting it; a key is created on first boot and nothing rotates it thereafter. |
...
| Caches do not cross tenants | The tool-response cache key covers the workspace, and a read compares the workspace recorded inside the entry with the one expected, in constant time, so any keying failure becomes a miss rather than a leak. Role bindings and audit policies are cached per workspace. | Yes |
...
| Configuration changes are versioned | `revisions` holds one row per change to a connector, a tool or an MCP server, written in the same transaction as the change itself, with a full snapshot and a diff. The revision number is allocated under an advisory lock inside that transaction, so two racing writers cannot both become revision four. | Yes — for those three kinds only; roles, identity providers and workspace settings are audited but not versioned |
...
| Graceful degradation | The rate limiter divides budgets per replica when Redis is gone rather than allowing traffic through, and reports it as a metric. The response cache resolves every error to a miss. The audit writer spools in memory and records the gap if it overflows. | Partial — an audit queue overflow drops events by default |
...
| Monitoring | Prometheus exposition on a separate listener with counts and latencies by connector type, status and error class, breaker state, audit queue depth and limiter health, plus six alert rules in the chart. | Yes |
| Tracing | **Not provided.** No OpenTelemetry instrumentation ships. |
...
| Retention limits | Per-workspace audit window, default 365 days, floor 90. Enforced hourly by scrubbing content and by a contiguous cut across the stream. | Partial — the audit stream only; see `retention.md` for the four stores nothing removes |
...
| No disclosure to the supplier | There is no telemetry, usage reporting, licence check or crash reporting in the server binary, and the adapter catalogue is compiled in, so browsing or installing an adapter reaches no network. | Yes |
...
- No data key rotation; master key rotation is manual. Signing keys
  rotate on their own, ninety days apart.
...
- The audit webhook is the only export destination.
