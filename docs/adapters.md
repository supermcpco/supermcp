...
| Field | Meaning |
|---|---|
| `name` | Lower-case snake_case, unique within the adapter. It *should* start with the slug with hyphens replaced by underscores (`nominatim_`); not doing so is a warning, because tool names collide across a catalogue of thousands. Names starting `supermcp_` are reserved for the instance's own tools, such as `supermcp_approval_status` (docs/api.md, "Following up a held call"), and refused (`tool-name-reserved`), on import and on every create or rename. |
...
| `tool-name-reserved` | error | Names starting `supermcp_` belong to the tools the instance adds to every server, which would shadow an adapter tool of the same name. |
